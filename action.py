...
def synchronize_into(name, from_path, to_path):
    # Clone 'from_path' into 'to_path', deleting 'to_path' first
    # if it doesn't exist.

    stdout(f'cloning {name} into {to_path}')
    ...
    runc(f'git clone {from_path} {to_path}', cwd=ARGS.workspace)
...
def main():
    ...
    zephyr_merge_base = get_merge_base(ARGS.workspace / 'zephyr',
                                       ZEPHYR_URL,
                                       branch='main')
    ncs_loot = get_ncs_loot(zephyr_merge_base,
                            ARGS.projects or DEFAULT_PROJECTS_TO_CHECK)

    for project, loot in ncs_loot.items():
        stdout('-' * 79)
        stdout(f'checking: {project}')
        from_path = ARGS.workspace / loot['path']
        to_path = ARGS.workspace / 'oss-history' / project
        synchronize_into(project, from_path, to_path)
